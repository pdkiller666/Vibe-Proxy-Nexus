...
>
> **Если у VPS только IP — создайте поддомен на основе вашего vpnexus.pro:**
>
> 1. Войдите на [reg.ru](https://reg.ru) → **Домены** → `vpnexus.pro` → **Управление DNS**.
> 2. Нажмите **Добавить запись** → тип **A**.
> 3. **Поддомен**: например `node-de`, **IP-адрес**: IP вашего VPS, **TTL**: 3600.
> 4. Сохраните. DNS обновится через 5–30 минут.
> 5. Вводите `node-de.vpnexus.pro` в поле «Домен» в форме авто-развертывания.
>
> **Рекомендованная схема именования:**
>
> | Поддомен | Назначение |
> |---|---|
> | `node-de.vpnexus.pro` | Германия |
> | `node-nl.vpnexus.pro` | Нидерланды |
> | `node-fi.vpnexus.pro` | Финляндия |
> | `node-ru.vpnexus.pro` | Россия |
>
> Одна A-запись = одна нода. Добавлять можно сколько угодно.
>
> **Нет собственного домена?** Альтернатива — бесплатный сервис [DuckDNS](https://duckdns.org):
> создайте поддомен вида `vpnde.duckdns.org` и укажите там IP VPS.
>
> *(Ручная настройка без домена описана ниже в Варианте B)*
