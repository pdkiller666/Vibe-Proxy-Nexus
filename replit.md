...
- **one-device-per-key enforcement**: all new VPN keys get `"limitIp": 1` in the Xray client entry (`addXrayClient` / `addRemoteXrayClient` in `keyIssuance.ts`). Existing keys issued before this feature was deployed retain no `limitIp` — `entrypoint.sh` copies the client list as-is on every container restart. `policy.levels["0"].connIdle` is set to 60 s so a device reconnecting from a new IP (e.g. Wi-Fi → LTE) is not blocked for 5 min while the old session times out. CGNAT caveat: multiple devices behind the same carrier NAT share one IP and bypass this limit.
