...
- `artifacts/api-server` — Express backend: routes under `src/routes/` (and `src/routes/admin/`), auth in `src/lib/auth.ts` (middlewares: `requireAuth` блокирует забаненных → 403 AccountBanned; `requireAuthAllowBanned` — без проверки `isBanned`, только для `GET /me`; `requireAdmin`) + `src/lib/session.ts`, VLESS link generation in `src/lib/vless.ts`, subscription-URL tokens in `src/lib/subscription.ts`, local Xray config management in `src/lib/xray.ts`, Xray client-config builder in `src/lib/xrayClientConfig.ts` (без geoip:ru тега — Happ iOS incompatible), traffic polling + key revocation from `src/lib/trafficPolling.ts`, hourly balance billing from `src/lib/hourlyBilling.ts`, balance-based checkout in `src/lib/balanceCheckout.ts`, auto-renewal job in `src/lib/autoRenew.ts`, balance-payment reconciler in `src/lib/reconcileBalancePayments.ts`, subscription auto-expiry/key-revocation in `src/lib/subscriptionLifecycle.ts`, node monitoring + auto-deactivation + metrics in `src/lib/nodeMonitoring.ts`, SSH provisioner in `src/lib/sshProvisioner.ts`, YooMoney webhook in `src/routes/yoomoney.ts`.
...
- `deploy/amvera-vpn-node/` — self-contained package for multi-region VPN nodes (separate VPS, Xray + management API). **Actively used**: live NL node on VDSina (`v917715.hosted-by-vdsina.com`). See `docs/add-vpn-node.md` for setup guide.
...
- `deploy/amvera-vpn-node/` is the multi-region VPN node package. Actively used alongside the all-in-one Amvera deployment — remote nodes managed via `remoteNode.ts` / `keyIssuance.ts`.
...
- **device slots, not transport limits**: each active VLESS key represents one intended device slot, and issuance is capped by `devicesIncluded + extraDeviceSlots`. New keys still carry `"limitIp": 1` only as a compatibility hint for a possible custom Xray image; the pinned vanilla Xray does not reliably enforce it. Raw WebSocket/session limits are deliberately disabled because Happ opens many parallel tunnels from one phone and rejecting those tunnels breaks page loading. See `docs/vpn-device-limits.md`.
- **Admin Audit Log**: every mutating admin action (POST/PATCH/PUT/DELETE → 2xx) is automatically written to `admin_audit_log` by `auditLogMiddleware` in `src/lib/auditLog.ts`. The middleware is registered in `admin/index.ts` as a **separate `router.use()` without a path** — Express 4 creates one Layer per callback in `router.use(path, fn1, fn2, fn3)`, and the third Layer is silently skipped (confirmed by diagnostic). No-path registration avoids this. The audit log UI is at Admin → «Журнал действий», cleaned up after 90 days by `auditLogCleanup.ts`. CSV export and filtering by admin/action/targetType/targetId/date range are supported. See `.agents/memory/amvera-finish-event-unreliable.md`.
- **Admin system events history**: `system_events` table stores both admin-scoped (userId IS NULL) and user-scoped (userId SET) events. The bell (`GET /api/admin/system-events`) returns only unacknowledged admin-scoped events. History tab (`GET /api/admin/system-events/history`) shows all admin-scoped events paginated with filters. `POST /api/admin/system-events/acknowledge-all` closes all at once. UI: Admin → «История событий» tab. Static sub-routes (`/acknowledge-all`, `/history`) must be registered before `/:id/acknowledge` in `admin/systemEvents.ts`. See `.agents/memory/admin-event-history.md`.
- **heal-schema.mjs**: runs before `drizzle-kit push` on every container start. Contains idempotent DDL patches (`ADD COLUMN IF NOT EXISTS`, unique indexes, FK ON DELETE rules, DROP COLUMN blocks) that drizzle-kit cannot apply unattended. All PL/pgSQL anonymous blocks use `DO $$ … $$` dollar-quoting (single `$` is invalid — PostgreSQL parses it as a positional parameter placeholder). Current migrations: M-0→M-15 (historical), M-16 (`users.is_banned`), M-17 (`users.referred_by_user_id` FK → ON DELETE SET NULL), M-24 (`vpn_nodes.cert_sha256`), M-36 (`admin_audit_log` table), M-37 (`support_messages.attachment_data`/`attachment_mime_type` text→text[]).
- **Auto-renewal**: `startAutoRenewJob()` in `src/lib/autoRenew.ts` runs hourly and tries to renew monthly subscriptions within 24 h of expiry via `checkoutFromBalance()`. On insufficient funds — emits a user-facing `system_event` notification; on technical error — emits an admin event. Dedup window = 24 h per user.
- **Node monitoring**: `startNodeMonitoringJob()` in `src/lib/nodeMonitoring.ts` polls remote nodes. After 3 consecutive failures: `isActive = false`, all active keys auto-migrated to other nodes, `node_unreachable` system_event written. On recovery: `isActive = true` restored, `node_recovered` event written. Metrics (CPU/RAM/disk) stored in `node_metric_snapshots` (max once/5 min per node, pruned after 90 days).
- **Invite links** (`invite_links` table): admin creates links with optional plan override, `maxUses`, `expiresAt`, `note`. `auth.ts` checks `invite_links.code` before `users.referral_code` — invite codes take priority and can force a specific plan on registration.
- **SSH auto-provisioning**: `POST /admin/vpn-nodes/:id/provision` triggers `sshProvisioner.ts` which SSHs into a VPS and installs Docker + Xray + management API. Progress streamed live via SSE (`GET /admin/vpn-nodes/:id/provision`). Logs persisted to `provisioning_jobs` table and survive server restarts.
...
- Admin panel (`/admin`, gated by role): pending payments queue (confirm/reject with screenshot preview), plans CRUD, VPN nodes CRUD, user management (activity status, ban/unban с бейджем «ЗАБЛ», force-logout, смена пароля, корректировка баланса, extra device slots, subscription override, приватные заметки), payment settings (SBP URL, phone/bank/recipient toggle, QR code upload, extra slot price, referral commission %, min hourly top-up, primary domain), support ticket queue, analytics, **«История событий»** (paginated system event history with type/date filters and per-event acknowledge), «Журнал действий» (admin audit log).
...
- Device-limit decision — `docs/vpn-device-limits.md`
