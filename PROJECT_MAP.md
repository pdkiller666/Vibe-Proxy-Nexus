...
- **`amvera.yml` не поддерживает `run.ports` (список)** — только одиночный
  `run.containerPort` (число, дефолт 80). Это узнали не сразу: сборка Docker
  проходила успешно (лог сборки — exit code 0, образ запушен), но отдельно в
  *логе приложения* висела «Configuration error: unknown fields ... ports» —
  контейнер с новым конфигом фактически не стартовал, отсюда постоянный
  503. **Урок**: чистый лог сборки ≠ валидный конфиг, всегда проверяй лог
  приложения после правок `amvera.yml`. Подробности — `.agents/memory/amvera-yaml-schema.md`.
- Через `containerPort` наружу (на публичный `*.amvera.tech` домен) идёт
  только ОДИН порт — веб/API (`8080`). Порт `443` (сырой TCP VLESS) этим не
  пробрасывается; для доступа к нему из интернета нужен отдельный аддон
  «Dedicated IPv4» в панели.
- В панели Amvera, раздел «Домены», есть форма ручного маппинга URL-путь →
  порт с предупреждением «не заполняйте, если один порт» — раз у нас
  `containerPort` теперь один (8080), это поле НЕ нужно заполнять; заполнение
  приводило к «Ошибке конфигурации».
- `env:` в `amvera.yml` тоже не поддерживается — все секреты только руками в
  панели, раздел «Переменные»/«Конфигурация».
