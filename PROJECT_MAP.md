...
Подписки автоматически истекают и отзывают ключи по окончании оплаченного
периода (с честным продлением при досрочной оплате), а на VPN-ноды можно
поставить лимит одновременных пользователей.
Есть админ-панель для управления тарифами, VPN-нодами (включая лимит
пользователей на ноду), платежами и ролями.
...
- `src/routes/` — все эндпоинты:
  ...
  - `me.ts` — `GET /api/me`, `PATCH /api/me` (имя), `PATCH /api/me/email`
    (требует текущий пароль, проверяет уникальность email),
    `PATCH /api/me/password` (требует текущий пароль, завершает остальные
    сессии пользователя)
  ...
  - `vpnKeys.ts` — `GET /api/vpn-keys/me`, `POST /api/vpn-keys`
    (требует активную подписку; если у узла задан `maxUsers`, полный узел
    исключается — явный `nodeId` на полном узле даёт `409`, автовыбор просто
    пропускает переполненные узлы),
  ...
  - `subscription.ts` — публичный `GET /api/sub/:token` (без авторизации,
    токен сам себя аутентифицирует, гейтится валидной активной подпиской):
    base64 всех активных ключей + заголовки
    `Profile-Title`/`Profile-Update-Interval`/`Subscription-Userinfo`
  - `health.ts` — `GET /api/healthz`
  - `admin/` — все под `requireAdmin`:
    `dashboard.ts` (сводка: онлайн сейчас, новые пользователи за 7/30 дней,
    распределение по тарифам, доход по дням за 14 дней),
    `users.ts` (список с вычисляемым `isOnline`/`lastActiveAt`, смена роли),
    `passwordReset.ts` (выдать ссылку сброса пароля пользователю),
    `payments.ts` (список/подтверждение/отклонение — транзакционно,
    идемпотентно, с честным продлением от текущей активной подписки),
    `plans.ts` (CRUD), `vpnNodes.ts` (CRUD; ответ включает вычисляемый
    `activeUserCount` — число неотозванных ключей на узле),
    `paymentSettings.ts` (обновление реквизитов СБП)
- `src/lib/`:
  ...
  - `session.ts` — БД-сессии: `createSession`/`destroySession`/`getUserBySessionToken`,
    ...
    сессий раз в час (`startSessionCleanupJob`); также троттлированно (раз в
    минуту на сессию) обновляет `users.lastActiveAt` при каждом обращении —
    от этого поля считается онлайн-статус (`ONLINE_THRESHOLD_MS` = 5 минут);
    юнит-тесты в `session.test.ts`
  - `subscriptionLifecycle.ts` — `startSubscriptionExpiryJob()` (запускается из
    `app.ts` рядом с очисткой сессий): периодически истекает просроченные
    подписки (`endsAt < now`) и отзывает VPN-ключи пользователя, но только
    если у него нет ДРУГОЙ ещё активной подписки (early renewal может дать
    несколько одновременно активных строк `subscriptions`); юнит-тесты в
    `subscriptionLifecycle.test.ts`. Lazy-expiry (`endsAt > now`) дополнительно
    проверяется defense-in-depth прямо в `meResponse.ts`, `vpnKeys.ts` и
    `subscription.ts` — просроченная подписка не даёт доступа, даже если
    джоба ещё не отработала.
...
- `src/pages/` — по одной странице на маршрут:
  ...
  `payments`, `profile` (смена имени/email/пароля, требует текущий пароль),
  `admin` (сводка с онлайн-статусом/аналитикой + все CRUD-разделы), `not-found`.
...
|---|---|---|
| `users` | email (unique), passwordHash, name, role (user/admin), lastActiveAt (nullable) | пользователи; `lastActiveAt` двигается троттлированно (раз/мин) при активности сессии — основа для онлайн-статуса |
...
| `vpn_nodes` | name, region, host, port (default 443), sni, publicKey, shortId, isActive, maxUsers (nullable = безлимит) | VPN-ноды; `activeUserCount` (число неотозванных ключей) вычисляется на лету в ответах API, в таблице не хранится |
