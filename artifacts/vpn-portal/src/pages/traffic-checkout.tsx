...
import { Copy, CheckCircle2, Clock, XCircle, RotateCcw, AlertTriangle, Gauge } from "lucide-react";
...
export default function TrafficCheckout() {
  ...
  const { mutate: cancelOrder, isPending: cancelling } = useDeleteExtraTrafficOrder();
  const { toast } = useToast();
  ...
  const statusConfig = {
    ...
    confirmed: { label: "Подтверждён — трафик добавлен", icon: CheckCircle2, color: "text-green-600" },
    ...
    <div className="max-w-xl space-y-8 animate-in fade-in duration-500">
      <div>
        <div className="flex items-center gap-2 text-muted-foreground mb-1">
          ...
          <span className="text-sm font-mono">Дополнительный трафик</span>
        </div>
        <h1 className="text-2xl font-bold tracking-tight">Оплата за доп. трафик</h1>
      ...
      {payment.status === "confirmed" && (
        <div className="bg-green-50 border border-green-200 p-4 text-sm text-green-800">
          Трафик начислен — если ключи были отозваны из-за лимита, они автоматически восстановлены.
        </div>
      )}
