...
// ─── cgroup v2 CPU: measures container-level usage_usec delta ─────────────────
...
// ─── Local system status — container-aware (cgroup v2 primary, /proc fallback) ─
async function getLocalSystemStatus() {

  // ── CPU ─────────────────────────────────────────────────────────────────────
  // Prefer cgroup v2 (container-level). Fall back to /proc/stat (host-level) if
  // cgroup is unavailable (e.g. older kernels or restricted environments).
  ...
  // ── RAM ─────────────────────────────────────────────────────────────────────
  // cgroup v2: memory.current = actual usage, memory.max = container limit.
  // "max" in memory.max means no cgroup limit is set → fall back to /proc/meminfo.
  ...
  // ── Disk ────────────────────────────────────────────────────────────────────
  // No standard cgroup v2 interface for disk capacity — keep df -B1 / which
  // reads the underlying filesystem. On Amvera this reflects the host disk;
  // for VPS nodes (bare VMs) it correctly shows the VPS disk.
  ...
  // ── Uptime ──────────────────────────────────────────────────────────────────
  // Use Node.js process uptime (seconds since last deploy/restart) rather than
  // /proc/uptime which on Amvera reflects the host machine (91+ days), not the
  // container lifecycle.
