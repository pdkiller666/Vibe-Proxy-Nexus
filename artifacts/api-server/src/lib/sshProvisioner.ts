...
/**
 * Generate a full HTTPS nginx config using a pre-existing certificate.
 * Used as a fallback when certbot cannot obtain a Let's Encrypt cert
 * (e.g. the hosting provider's DNS returns SERVFAIL for CAA queries,
 * causing Let's Encrypt to refuse issuance).
 */
function makeNginxHttpsConfig(domain: string, certPath: string, keyPath: string): string {
  return `# Auto-generated by VPNexus provisioner — do not edit manually.
# Self-signed certificate fallback (certbot unavailable).
...
async function provisionAsync(job: ProvisioningJob, opts: ProvisioningOpts): Promise<void> {
  ...
  try {
    ...
    // NOTE: no apostrophes in ufw comment strings — single quotes cannot be
    // escaped inside single-quoted bash strings, causing a syntax error.
    emitStep(job, "🔒 Настройка UFW (открываем 80, 443, 8443)...");
    const ufwSetup = [
      "ufw allow 22/tcp comment SSH 2>/dev/null || true",
      "ufw allow 80/tcp comment HTTP-LetsEncrypt 2>/dev/null || true",
      "ufw allow 443/tcp comment HTTPS-VPN 2>/dev/null || true",
      "ufw allow 8443/tcp comment VPN-MgmtAPI 2>/dev/null || true",
    ...
    // ── Step 9: TLS certificate (Let's Encrypt with self-signed fallback) ────
    emitStep(job, `🔐 Получение TLS-сертификата для ${domain}...`);
    ...
    try {
      ...
      await writeRemoteFileViaExec(conn, "/tmp/vpn-node-nginx-https.conf", nginxHttpsConfig, job);
      await runCommand(
        conn,
        [
          "cp /tmp/vpn-node-nginx-https.conf /etc/nginx/sites-available/vpn-node",
          "nginx -t",
          "systemctl restart nginx",
        ].join(" && "),
        job,
    ...
    // Health check via the Management API directly on localhost (avoids
    // dependence on nginx TLS being set up correctly, and works for both
    // Let's Encrypt and self-signed scenarios).
    const healthOut = await runCommand(
      conn,
      `curl -fsS --max-time 10 http://localhost:8443/health`,
      job,
      { timeoutMs: 30_000 },
    );
