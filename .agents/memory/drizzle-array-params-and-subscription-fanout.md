...
Same trap applies to `sql<Date>\`max(timestamptz_col)\`` in a `groupBy` aggregate: the generic is a type assertion only, and the driver can hand back something that isn't a real `Date` instance, so `.getTime()` throws at runtime (`x.getTime is not a function`) with zero compile-time warning. Caught only by a manual DB-backed test run of a batch/job query, not by `tsc`. Always `new Date(value)` before calling Date methods on any raw `sql` aggregate result, regardless of its declared generic type.
