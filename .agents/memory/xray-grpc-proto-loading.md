...
**Critical build packaging rule:** esbuild bundles JS only — `.proto` files loaded at runtime via `protoLoader.loadSync(__dirname + "/xray-proto/...")` are NOT bundled automatically. They must be explicitly copied to `dist/` in `build.mjs` (using `fs.cp`) or protoLoader throws ENOENT in production on every call, with the error silently swallowed by the polling catch block. This was the root cause of `pollUserTrafficDeltas: failed to query Xray StatsService` persisting in production even after the Xray config was correctly updated. Fix confirmed working after adding the `cp` step in `artifacts/api-server/build.mjs`.
